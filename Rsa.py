...
def criptografar(mensagem: int, chave: tuple[int, int]) -> int:
    cif = pow(mensagem, chave[0], chave[1])
    return cif


def descriptografar(mensagem_cifrada: int, chave: tuple[int, int]) -> int:
    msg = pow(mensagem_cifrada, chave[0], chave[1])
    return msg


def gerarAssinatura(mensagem: str, chave: tuple[int, int]):
